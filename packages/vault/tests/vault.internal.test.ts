...
describe('Vault internal coverage', () => {
  describe('resolvability certification', () => {
    it('validates a repeated successful sealed local root only once', () => {
      const Root = token<number>('CertifiedResolvableRoot');
      const vault = new Vault({
        providers: [{ provide: Root, useFactory: () => 1 }],
      });
      ...
      expect(vault.canResolve(Root)).toBe(true);
      expect(vault.canResolve(Root)).toBe(true);

      expect(validate.mock.calls.filter(([canonical]) => canonical === Root.id)).toHaveLength(1);
      expect(vault.store.getByCanonical(Root.id)!.flags & (1 << 8)).toBe(1 << 8);
    });

    it('revalidates failures and keeps cycles and lifecycle violations false', () => {
      const Missing = token<unknown>('UncertifiedMissingDependency');
      const NeedsMissing = token<unknown>('UncertifiedMissingRoot');
      ...
      const missingInternal = missingVault as unknown as {
        _validateResolvableGraph(
          canonical: CanonicalId,
          path: ResolutionPath,
          isRoot: boolean
      ...
      const missingValidate = vi.spyOn(missingInternal, '_validateResolvableGraph');

      expect(missingVault.canResolve(NeedsMissing)).toBe(false);
      expect(missingVault.canResolve(NeedsMissing)).toBe(false);
      expect(
        missingValidate.mock.calls.filter(([canonical]) => canonical === NeedsMissing.id)
      ).toHaveLength(2);
      expect(missingVault.store.getByCanonical(NeedsMissing.id)!.flags & (1 << 8)).toBe(0);

      const CycleA = token<unknown>('UncertifiedCycleA');
      const CycleB = token<unknown>('UncertifiedCycleB');
      const cycleVault = new Vault({
        providers: [
          { provide: CycleA, deps: [CycleB], useFactory: () => 'a' },
          { provide: CycleB, deps: [CycleA], useFactory: () => 'b' },
        ],
      });
      expect(cycleVault.canResolve(CycleA)).toBe(false);
      expect(cycleVault.canResolve(CycleA)).toBe(false);
      expect(cycleVault.store.getByCanonical(CycleA.id)!.flags & (1 << 8)).toBe(0);

      const Transient = token<unknown>('UncertifiedTransientDependency');
      const Singleton = token<unknown>('UncertifiedSingletonRoot');
      const lifecycleVault = new Vault({
        providers: [
          {
            provide: Singleton,
            deps: [Transient],
      ...
      expect(lifecycleVault.canResolve(Singleton)).toBe(false);
      expect(lifecycleVault.canResolve(Singleton)).toBe(false);
      expect(lifecycleVault.store.getByCanonical(Singleton.id)!.flags & (1 << 8)).toBe(0);
    });
  });

  describe('invalidates certification', () => {
    it('clears a local root certificate when a lazy cross-Vault attachment materializes', () => {
      const LocalDependency = token<number>('LazyCertificateLocalDependency');
      const ForeignDependency = token<number>('LazyCertificateForeignDependency');
      const Root = token<number>('LazyCertificateConsumer');
      const producer = new Vault({
        providers: [{ provide: ForeignDependency, useValue: 2 }],
      ...
      expect(consumer.canResolve(Root)).toBe(true);
      expect(rootEntry.flags & (1 << 8)).toBe(1 << 8);
