...
describe('Lifecycle validation (resolution-time)', () => {
  it('catches singleton depending on scoped even when consumer registered BEFORE dependency', () => {
    ...
    class BadSingleton {
      constructor(@Summon(ScopedToken) _dep: unknown) {}
    }
    ...
    const vault = new Vault({ relics: [BadSingleton, ScopedDep], shadowPolicy: 'allow' });
  ...
  it('catches singleton depending on transient in reverse registration order', () => {
    ...
    @Relic({ provide: SingletonToken, lifecycle: Lifecycle.Singleton })
    class BadSingleton2 {
      constructor(@Summon(TransientToken) _dep: unknown) {}
    }
    ...
    const vault = new Vault({ relics: [BadSingleton2, TransientDep], shadowPolicy: 'allow' });
